...
if (isset($_POST['add'])) {
    ...
    $exists  = mysqli_fetch_assoc(mysqli_query($conn, "SELECT category_id FROM gallery_category WHERE category='$heading' AND category_id != '$id' LIMIT 1"));
    if ($exists) {
...
$qry = mysqli_query($conn, "SELECT * FROM gallery_category WHERE category_id='$id' LIMIT 1");
...
<head>
  ...
  <title>Edit Gallery Category – Singaliner Admin</title>
...
<div class="page-hdr"><div><h1>Edit Gallery Category</h1></div></div>
