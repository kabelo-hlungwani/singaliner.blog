...
echo '<script>alert(" category Exist,try new category.");window.location = "s-category.php";</script>';

}else
{


$edit=mysqli_query($conn,"UPDATE gallery_category SET category='$heading' where category_id='$id'");  


if($edit){
mysqli_close($conn);

echo '<script>alert(" category updated.");window.location = "g-category.php";</script>';
...
<body>
    <div class="container shadow-lg" data-aos="fade-down-right" data-aos-duration="900" data-aos-delay="500" style="font-family: Barlow, sans-serif;padding-bottom: 46px;">
        <form method="post" style="margin-top: 50px;">
            ...
            <h2 class="text-center">Edit Images categories</h2><label>Category</label>
            <div class="form-group"><input class="form-control" type="text" value="<?php  $qry=mysqli_query($conn,"SELECT * FROM gallery_category where category_id='$id'");
 $dat=mysqli_fetch_array($qry);
 echo $dat['category']
 ?>" name="heading" onkeydown="return /[a-z, ]/i.test(event.key)"
